...
export class TransactionService {
	...
	public async makePayment(user: User, planId: string) {
		...
		if (user.id === plan.channel.id) {
			throw new ConflictException("You can't buy your own plan")
		}
		...
		const successUrl = `${this.configService.getOrThrow<string>('ALLOWED_ORIGIN')}/success?price=${encodeURIComponent(plan.channel.username)}`
